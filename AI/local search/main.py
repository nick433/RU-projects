...
def main():
    ...
    while(True):
        while(goodInput == False):
            ...
            if(x == '6'):
                print('\nThis uses number in puzzleSize.txt\n')
            ...
            if(x == '7'):
                ...
                x = geneticAlgorithm(int(steps), int(pop), int(iPop), float(selPressure), float(mutProb), float(crossProb), puzSize) #should temp be a float?
                printPuzzle(x[0][1])
                print()
                printPuzzle(x[0][1],True)
                #print(x)
                print("This puzzle evaluates to: " + str(x[0][0]))
                print("The first k was " + str(x[1]))
            if (x=='8'):
                return 0
...
def hillClimbEval(n,board = None):
    ...
    print("Time elapsed in milis: " + str(milisElapsed))
...
# 2000 iters, 1000 temp, .995 rate, try .999 next since after 55k iterations we are at 1e-120            
    ...
    print("Newest board's min distances from start:\n")
    printPuzzle(board,True)
    print("Evaluation function is " + str(l[-1]))
    print()
    #print()    
...
def geneticAlgorithm(steps, pop = 2, iPop = 4, selPressure = 0.5, mutProb = 1, crossProb = 0.5, psize = 5):
    ...
    if bestGuy[0] <= lastgen[-1][0]:
        return (lastgen[-1],oldK) # (k,bestboard)
    else:
        return (bestGuy,firstK)
